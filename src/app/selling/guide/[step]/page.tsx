...
export async function generateStaticParams() {
  return sellingGuideSteps.map((step) => ({ step: step.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { step: stepSlug } = await params;
  const step = sellingGuideSteps.find((s) => s.slug === stepSlug);
  if (!step) return {};
  return {
    title: `${step.title} | Vancouver Seller's Guide`,
    description: step.description,
  };
}

export default async function SellingGuideStepPage({ params }: Props) {
  const { step: stepSlug } = await params;
  const step = sellingGuideSteps.find((s) => s.slug === stepSlug);
  if (!step) notFound();

  const prevStep = sellingGuideSteps.find((s) => s.step === step.step - 1);
  const nextStep = sellingGuideSteps.find((s) => s.step === step.step + 1);
      ...
      <section className="bg-teal-950 py-12">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-teal-300/70 mb-4">
            <Link href="/selling" className="hover:text-teal-200 transition-colors">Selling</Link>
            <span>/</span>
            <Link href="/selling/guide/deciding-to-sell" className="hover:text-teal-200 transition-colors">Guide</Link>
            <span>/</span>
      ...
      <div className="bg-warm-50 border-b border-warm-100">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="flex gap-1.5">
            {sellingGuideSteps.map((s) => (
              <Link
                key={s.slug}
                href={`/selling/guide/${s.slug}`}
