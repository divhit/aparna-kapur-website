...
function PlacesSearchInner({ query, neighbourhood, lat, lng }: Props) {
  ...
  useEffect(() => {
    ...
    const initRequest = () => {
      try {
        ...
        reqElement.locationBias = new google.maps.Circle({
          center: { lat, lng },
          radius: 2000,
        });
      } catch {
  ...
  if (!apiKey) {
    return (
      <div className="bg-warm-50 rounded-xl p-3 my-2 text-xs text-warm-500 italic">
        Places search unavailable. Contact Aparna directly for local
        recommendations!
      </div>
    );
  }
